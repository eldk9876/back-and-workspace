/*
 * * 자바의 특징
 * - 객체지향 프로그래밍(Object_Oriented Programming,OOP) 언어이다.
 ...
 * - 번역된 바이트코드(기게어)를 JVM의 자바 인터프리터(java.exe)를 이용해 실행한다.
 * 
 * * 패키지 (package)
 * - 폴더를 만들어서 저장하고 관리하듯이 패키지를 만들어 클래스를 저장하고 관리한다.
...
/*
 * 클래스(class)
 * - 자바에서 모든 코드는 반드시 클래스 안에 존재해야 하며 서로 관련된 코드들을 그룹으로 나누어 
 * 	 별도의 클래스를 구성한다.
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	...
	public static void main(String[] args) {
		...
		System.out.println("Hello, Java!"); // ctrl +  F11 출력 단축키
	...
	/*
	 * 네이밍 규칙
	 * - 패키지 : 반드시 소문자로 작성, 회사들 간에 패키지가 서로 죽복되지 않도록 
