...
*/
	...
	SELECT SUBSTR('PROGRAMMING', -8, 3); -- GRA 마이너스(-)는 뒤에서 부터 
...
/*
	...
	UPPER : 다 대문자로 변견한 문자열 반환
*/
    SELECT LOWER('Welcome To MySQP'), UPPER('Welcome To MySQP');
...
-- 2. 사원명과 주민등록번호(000000-0******)으로 조회
	SELECT emp_name, emp_no, REPLACE(emp_no, SUBSTR(emp_no , 9, 6) ,'******')
    FROM employee;
...
-- 3. 사원명, 이메일, 이메일에서 추출한 아이디 조회 (@ 앞) 
	SELECT emp_name, email, REPLACE(email , SUBSTR(email , -9, 9), '') '추출한아이디' -- SUBSTR(email , -9, 9)
	 FROM employee;
...
*/
	...
	SELECT ROUND(123.567), ROUND(123.567, 1), ROUND(123.567, 2), ROUND(123.567, -1), ROUND(123.567, -2);
...
/*
    ...
    DAYOFMONTH : 날짜가 해당 월에서 몇 번쨰 날인짖 반환
...
*/
    ...
    DATE_FORMAT(NOW(), '%Y.%m.%d %T'); -- %Y : 시간 전체, %m : 시, %d : 분, %T : 초
...
-- 직원명, 부서코드(dept_code) 조회 (부서코드가 없으면 '부서없음')
	SELECT emp_name, dept_code, IFNULL(dept_code, '부서없음')
    FROM employee;

/*
	NULLIF(값1, 값2)
    - 두 개의 값이 동일하면 NULL반환, 두 개의 값이 동일하지 않으면 값1 반환
*/
...
-- 사번(emp_id), 사원명, 주민번호(emp_no), 성별(남,여) - emp_no 활용해서! 조회
	SELECT emp_id, emp_name, emp_no, SUBSTR(emp_no , 8, 1) -- if(emp_no, SUBSTR(emp_no , 8, 1) = 1 , SUBSTR(emp_no , 8, 1) = 2) 
    FROM employee;

-- 강사님 풀이 
	SELECT emp_id, emp_name, emp_no, SUBSTR(emp_no , 8, 1), if(SUBSTR(emp_no , 8, 1) = 1 ,'남자', '여자') '성별' 
    FROM employee;
/*    
...
*/
	-- 인상된 급여만 만들어서 정렬시키면 끝
	SELECT emp_name, job_code, FORMAT(salary, 0), 
		/*	 if((job_code= 'J7'),(salary * 1.1), (salary * 1.05) ) 'J7',
             if((job_code= 'J6'),(salary * 1.15),(salary * 1.05) ) 'J6',
             if((job_code= 'J5'),(salary * 1.2), (salary * 1.05) ) 'J5'*/
          --  IF((job_code. J7),(salary * 0.1), (salary * 0.05),(job_code. J6),(salary * 0.15),(salary * 0.05),(job_code. J5),(salary * 0.2), (salary * 0.05)  )
 -- 교수님 코드
...
-- salary 값이 500안원 이하 200만원 초과일 경우 3등급
-- 그 외의 경우 4등급

	SELECT emp_name, salary,
	/*	CASE WHEN salary >= 5000000 THEN 3500000 < salary 
			 ELSE salary >= 5000000 THEN 2000000 < salary 
        END*/
-- 강사님 코드
...
/*
	AVG
    - 해당 컬럼 값들의 평균값을 반환
    - 모든 그룹 함수는 NULL 값을 자동으로 제외하기 때문에 AVG 함수를 사용할 때는 COALESCE 또는 IFNULL 함수와 함께 사용하는걸 권장
*/
