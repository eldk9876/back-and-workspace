/*
	DML(Date Manipulation Lenguage)
    ...
    - 테이블에 모든 컬럼에 대한 값을 INSEERT 하고자 할 떄 사용한다.
    ...
    - 서브쿼리 결과값이 INSERT 문에 지정된 테이블 컬럼 개수와 같라야 한다.
*/
-- 사용할 테이블 생성
	CREATE TABLE emp(
...
-- 모튼 사원의 급여를 기존 급여에서 10 프로 인상한 금액(기존 급여 * 1.1)으로 변경
	UPDATE emp_salary
...
-- 아시아 지역에 근무하는 직원들의 보너스를 0.3으로 변경 (변경 사항 테이블은 emp_salary)
	SELECT * 
    FROM emp_salary
    JOIN kh.employee USING(dept_code)
	JOIN kh.department ON (dept_code = dept_id )
    ...
    SELECT * 
    FROM emp_salary in (UPDATE emp_salary 
						SET bonus = 0.3
						WHERE local_name LIKE 'ASIA%')
    JOIN kh.employee USING(dept_code)
	JOIN kh.department ON (dept_code = dept_id )
    ...
    JOIN kh.department ON (dept_code = dept_id ) 
    JOIN kh.location  ON (location_id = local_code)
	JOIN emp_salary  = (UPDATE emp_salary 
						SET bonus = 0.3
						WHERE local_name LIKE 'ASIA%')  USING(dept_code) 
    
   
	;
    
	UPDATE emp_salary in (emp_salary 
						SET bonus = 0.3
						WHERE local_name LIKE 'ASIA%')
    JOIN kh.department ON (dept_code = dept_id ) 
    JOIN kh.location  ON (location_id = local_code)
	JOIN emp_salary  USING(dept_code) 
