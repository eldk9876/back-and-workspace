...
*/
-- 사원의 사번, 사원명, 직급명, 성별(남/여),근무년수를 조회할 수 있는 SELECT 문을 뷰(vw_emp_job)생성
	CREATE OR REPLACE VIEW vw_emp_job
    AS SELECT  emp_id '사번', emp_name '사원명', job_name '직급명' , 
    if(Substr(emp_no,8 ,1 )=1,'남','여') '성별', timestampdiff(year, hire_date,now()) '근무년수' 
    FROM employee
...
*/
    ...
    SELECT * FROM job; -- 베이스 테일블 (실제 데이터가 담겨있음)
...
-- 1) 뷰 정의에 포함되지 않은 컬럼을 조작하는 경우
	...
	-- 2) 뷰에 포함되지 않은 컬럼 중에 베이스가 된는 컬럼이 NOTNULL 제약조건이 지정된 경우
...
-- 사번, 사원명, 급여, 연봉(salary * 12)을 조회한 SELECT 문으로 vw_emp_sal 뷰 정의
    ...
    VALUES(301, '홍길동','000000-0000000', 3000000); -- 확인 필요
...
-- 부서별 급여의 합계, 평균을 조회한 SELECT 문을 vw_groupdept 뷰 정의
	...
	AS SELECT dept_code, format(SUM(Salary),0) '합계', format(avg(Salary),0)'평균'
    FROM employee
	GROUP BY dept_code;
